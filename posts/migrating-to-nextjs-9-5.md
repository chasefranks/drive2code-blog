...
In any event, it makes sense, so we carried on. The pattern we ended up with that helped was to wrap the styling in a :global block with an outer local class name
